...
@WebMvcTest(
        controllers = PersonalityController.class,
        properties = {
...
class PersonalityControllerTest {
    ...
    void rejectsUnsupportedAnswerValueWithPersonalityError() throws Exception {
        String invalidRequest = validRequest().replace("\"value\": 1", "\"value\": 2");
    ...
    void submitsValidProfileUsingJwtUserId() throws Exception {
        UUID userId = UUID.randomUUID();
        PersonalityProfileResponse response = PersonalityProfileResponse.incomplete(
                PersonalityOnboardingStatus.COMPLETED
        );
        when(personalityService.upsertProfile(eq(userId), any())).thenReturn(response);
