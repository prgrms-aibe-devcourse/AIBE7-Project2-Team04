...
- **설명 가능성과 재현성**: 희망 태그와 자유 텍스트 임베딩의 가중치 및 호환도 함수는 애플리케이션의 버전된 규칙으로 관리하고 `match_requests.matching_formula_version`을 남깁니다. 구버전 성향 벡터는 버전 계열 검사에서 제외하며, 동의·자기소개가 유효한 프로필과 활성 요청만 비동기 배치로 재생성합니다. LLM 응답을 후보 탈락의 단독 근거로 사용하지 않습니다.
