...
- **Redis 관리 대상**:
  ...
  - 5분 대기 시간(TTL) 및 사용자별 매칭 진행 대기 상태(`match:user:{user_id}`)
...
| `preference_snapshot` | JSONB | NULL | 요청 시점의 네 가지 성향 차원별 중요도와 유사·보완 선호를 담은 `MatchingPreferenceSnapshot` |
...
| `score_snapshot` | JSONB | NULL | 정렬된 요청 기준 `request1 → request2`, `request2 → request1` 점수·사유, 최종 쌍 점수와 산식 버전 |
...
- 성향 조사 결과와 분리하여 매칭 도메인이 소유하며, 사용자가 매칭 시작 전에 정한 상대 성향의 차원별 중요도와 유사·보완 선호를 저장합니다.
- 선호를 설정한 사용자는 네 가지 차원을 각각 한 행으로 저장하며, API 갱신 시 네 행을 단일 트랜잭션에서 전체 교체합니다.
- 저장된 행이 없는 사용자는 기본 선호를 임의 생성하지 않고, 매칭 랭킹에서 사용자별 성향 차원 점수를 적용하지 않습니다. 매칭 요청 생성 시 현재 네 행을 `match_requests.preference_snapshot`에 복사합니다.
