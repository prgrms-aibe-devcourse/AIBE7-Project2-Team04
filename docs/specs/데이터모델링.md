...
| 16 | 사용자 매칭 선호 | `user_matching_preferences` | 상대 성향 차원별 중요도와 유사·보완 선호 |
| 17 | 사용자 기본 활동지역 | `user_location_preferences` | 구 단위 활동지역과 위치 기반 서비스 동의 |
| 18 | 매칭 요청 희망 성향 태그 | `match_request_desired_personality_tags` | 매칭 요청별 원하는 상대의 `PersonalityTag` 목록 |
...
| `source_text` | TEXT | NOT NULL | 카드 점수·확정 태그·자기소개로 구성한 버전 문서. 상대방에게 노출하지 않음 |
...
#### 16. `user_matching_preferences` (사용자 매칭 선호)
...
| `updated_at` | TIMESTAMPTZ | NOT NULL | 선호 수정 시각 |
...
#### 17. `user_location_preferences` (사용자 기본 활동지역)
