...
- 매칭 도메인이 소유하는 사전 설정으로, 저장 시 사용자별 네 차원을 모두 유지하며 API에서 단일 트랜잭션으로 전체 교체합니다. 미설정 사용자는 행이 없고 사용자별 성향 차원 점수를 적용하지 않습니다. 요청 생성 시 `match_requests.preference_snapshot`으로 복사합니다.
...
- PK: `id BIGINT`
- FK: `user_id → users.id`
- 컬럼: `food_category`, `meal_at`, `region_code`, `region_name`, `location_name`, `location GEOGRAPHY`, `search_radius`, `desired_personality_text`, `desired_personality_embedding vector(1536)`, `embedding_model`, `embedding_version`, `embedded_at`, `preference_snapshot JSONB`, `matching_formula_version`, `status`, `reject_count`, `created_at`, `updated_at`
...
- `preference_snapshot`은 네 가지 성향 차원을 모두 포함하는 `MatchingPreferenceSnapshot` JSON 구조입니다.

### 10-1. `match_request_desired_personality_tags`
...
- `score_snapshot`은 두 방향의 점수·사유, 최종 쌍 점수와 산식 버전을 정렬된 요청 순서로 보존합니다.
- 수락 대기 시간: 15초
...
- `match:user:{user_id}`: 사용자별 중복 대기 차단, TTL 5분
- `match:waiting:{request_id}`: 요청별 대기 상태와 잔여 TTL, TTL 5분
