...
- 컬럼: `message_type(TEXT, PLACE)`, `content TEXT`, `provider_place_id`, `place_name`, `place_category`, `place_address`, `place_location GEOGRAPHY(POINT,4326)`, `place_url`, `created_at`
- `TEXT` 메시지의 장소 컬럼은 모두 NULL이고, `PLACE` 메시지의 장소 컬럼은 모두 필수입니다.
...
- 신고 행은 관리자 처리 후에도 삭제하지 않고 상태만 변경하여, 동일 사용자가 동일 매칭에서 신고를 다시 제출하지 못하도록 합니다.
