...
- PK: `id UUID`
- 컬럼: `email`, `password_hash`, `provider`, `provider_id`, `nickname`, `profile_image_url`, `profile_image_key`, `description`, `role`, `status`, `personality_onboarding_status`, `created_at`, `updated_at`
...
기존 DB 배포 전에는 [`매칭_참여자_역할_및_ID_정합성.sql`](../migrations/매칭_참여자_역할_및_ID_정합성.sql)을 실행하며,
후기 API 전환 전에는 [`사용자_후기_컬럼_전환.sql`](../migrations/사용자_후기_컬럼_전환.sql)을 추가로 실행합니다.
검증 절차는 [`docs/migrations/README.md`](../migrations/README.md)를 따릅니다.
