...
- 자유 서술·Spring AI 임베딩·실제 매칭 랭킹은 MVP 이후 단계로 분리한다.
...
- [ ] `/personality/survey` 온보딩 페이지와 로그인 이후 진입 경로를 추가한다.
- [ ] `project2.isLoggedIn` 플래그가 아니라 성향 조회 API 결과로 온보딩 필요 여부를 판단한다.
- [ ] 대화 분위기, 식사 속도, 약속 스타일, 메뉴 탐색 스타일 카드 단일 선택을 구현한다.
- [ ] 세부 스타일 태그를 최대 5개까지 선택하는 UI를 구현한다.
- [ ] 음식 카테고리를 최대 5개까지 선택하는 UI를 별도 단계로 구현한다.
- [ ] 단계별 진행 상태, 필수 선택 누락, 최대 선택 개수 안내를 구현한다.
- [ ] 제출 전에 `GET /auth/csrf`를 호출하고 `X-XSRF-TOKEN` 헤더를 전달한다.
- [ ] 인증 쿠키가 전송되도록 모든 관련 요청에 `credentials: 'include'`를 적용한다.
- [ ] 제출 성공 후 홈 또는 다음 온보딩 화면으로 이동한다.
- [ ] 건너뛰기 API를 호출하여 `SKIPPED`를 저장하고 기본 매칭 이용을 차단하지 않는다.
- [ ] `401`이면 로그인 화면, 입력 오류이면 해당 단계에 한국어 메시지를 표시한다.

## 5. 저장·보안·Fallback 규칙

- [ ] 원본 카드 응답과 계산 점수를 함께 저장하여 V1 점수를 재현할 수 있게 한다.
- [ ] 재제출 시 기존 답변과 태그를 남겨두지 않고 전체 갱신한다.
- [ ] 음식 선호 갱신도 요청에 포함된 값으로 전체 교체한다.
- [ ] 초기화 시 프로필, 답변, 태그, 동의한 자유 서술 및 파생 임베딩을 함께 제거한다.
- [ ] 원본 응답과 자유 서술은 본인에게만 제공하고 상대방이나 로그에 노출하지 않는다.
- [ ] 카드 점수의 높고 낮음을 좋은·나쁜 성향으로 표현하지 않는다.
- [ ] 성향 프로필이나 음식 선호가 없어도 기본 조건 매칭이 정상 동작하도록 한다.

## 6. 검증 및 문서 동기화

- [ ] 응답값 `1/3/5`가 점수 `0/50/100`으로 계산되는지 테스트한다.
- [ ] 차원 누락·중복, 허용하지 않는 응답값, 지원하지 않는 버전 테스트를 작성한다.
- [ ] 태그·음식 카테고리 최대 개수와 잘못된 코드 검증 테스트를 작성한다.
- [ ] 최초 제출·재제출·조회·초기화 서비스 테스트를 작성한다.
...
- [ ] 최대 300자의 선택형 자기 스타일 설명과 `aiAnalysisConsent`를 추가한다.
- [ ] 동의한 자유 서술에 대해서만 기존 태그 코드 중 관련 태그를 AI가 제안한다.
- [ ] AI 제안 태그는 자동 확정하지 않고 사용자가 확인·수정한 값만 저장한다.
- [ ] 기본 카드·확정 태그·자기소개를 버전된 문서로 구성하여 비동기로 임베딩한다.
- [ ] 임베딩과 함께 `model_name`, `source_version`, `generated_at`을 저장한다.
- [ ] 동의 철회 시 자유 서술 원문과 파생 임베딩을 삭제한다.
- [ ] AI 실패가 정형 프로필 저장이나 기본 매칭을 실패시키지 않도록 한다.
- [ ] 자유 서술, AI 요청·응답 및 임베딩 원문을 로그에 남기지 않는다.
