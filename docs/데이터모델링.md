...
1. **영구 데이터베이스**: PostgreSQL (Supabase)
   - 사용자 계정과 Refresh Token 세션, 위치·성향 정보, 실시간 1:1 매칭 요청·결과·참여자, 채팅방/메시지, 식당 추천 정보와 매너 후기 등 장기 보존이 필요한 데이터를 관리합니다.
2. **위치 데이터 처리 (PostGIS)**:
...
| 9 | 매너 후기 | `user_reviews` | 매칭 상대방에 대해 작성한 매너 및 평점 후기 |
...
| 13 | 사용자 성향 임베딩 | `user_personality_embeddings` | 동의한 자유 서술 성향의 보조 랭킹 벡터 |
| 14 | 사용자 매칭 선호 | `user_matching_preferences` | 상대 성향 차원별 중요도와 유사·보완 선호 |
| 15 | 사용자 기본 활동지역 | `user_location_preferences` | 구 단위 활동지역과 위치 기반 서비스 동의 |
...
- *Check Constraint*: 평점은 1~5이고 작성자와 평가 대상은 서로 달라야 합니다.
...
#### 13. `user_personality_embeddings` (사용자 성향 임베딩)
...
#### 14. `user_matching_preferences` (사용자 매칭 선호)
...
#### 15. `user_location_preferences` (사용자 기본 활동지역)
...
- 동의 철회 시 기본 활동지역을 삭제하고 이후 위치 기반 요청을 거부합니다. 기존 매칭 요청 핀의 보존·삭제는 매칭 이력 보존 정책을 따릅니다.
...
CREATE TABLE user_food_preferences (
    ...
    PRIMARY KEY (user_id, food_category)
);

-- 15. 사용자 성향 임베딩 테이블 (user_personality_embeddings)
...
-- 16. 사용자 매칭 선호 테이블 (user_matching_preferences)
...
-- 17. 사용자 기본 활동지역 테이블 (user_location_preferences)
