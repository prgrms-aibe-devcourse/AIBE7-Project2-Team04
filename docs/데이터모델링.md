...
2. **위치 데이터 처리 (PostGIS)**:
   ...
   - 프론트엔드는 구 대표 좌표로 지도를 초기화하고, 사용자가 확정한 핀은 실시간 매칭 요청(`MATCH_REQUEST.location`)과 게시판 매칭 장소(`MATCH_POST.location`) 등 필요한 시점에만 `GEOGRAPHY(POINT, 4326)`로 저장합니다.
...
| 16 | 사용자 성향 프로필 | `user_personality_profiles` | 자가 응답에서 계산한 버전별 정형 성향 점수 |
| 17 | 사용자 성향 응답 | `user_personality_answers` | 재계산과 사용자 수정에 필요한 설문 응답 |
| 18 | 사용자 성향 임베딩 | `user_personality_embeddings` | 동의한 자유 서술 성향의 보조 랭킹 벡터 |
| 19 | 사용자 매칭 선호 | `user_matching_preferences` | 상대 성향 차원별 중요도와 유사·보완 선호 |
| 20 | 사용자 기본 활동지역 | `user_location_preferences` | 구 단위 활동지역과 위치 기반 서비스 동의 |
...
- **필요 시점의 저장**:
  ...
  - 모집 게시글에서 사용자가 확정한 약속 장소 핀: `match_posts.location`
...
| `region_code` | VARCHAR(5) | NOT NULL | 약속 핀이 속한 5자리 시·군·구 코드 |
| `region_name` | VARCHAR(100) | NOT NULL | 서버 기준 데이터로 정규화한 구 표시명 |
...
#### 16. `user_personality_profiles` (사용자 성향 프로필)
...
#### 17. `user_personality_answers` (사용자 성향 응답)
...
#### 18. `user_personality_embeddings` (사용자 성향 임베딩)
...
#### 19. `user_matching_preferences` (사용자 매칭 선호)
...
#### 20. `user_location_preferences` (사용자 기본 활동지역)
...
-- 16. 사용자 성향 프로필 테이블 (user_personality_profiles)
...
-- 17. 사용자 성향 설문 응답 테이블 (user_personality_answers)
...
-- 18. 사용자 성향 임베딩 테이블 (user_personality_embeddings)
...
-- 19. 사용자 매칭 선호 테이블 (user_matching_preferences)
...
-- 20. 사용자 기본 활동지역 테이블 (user_location_preferences)
...
CREATE INDEX idx_match_posts_region_status ON match_posts(region_code, status);
...
2. **매칭 탐색 및 매칭 시도 (Matching)**:
   ...
   - 위치·시간·인원·대기 상태·차단 관계의 하드 필터를 통과한 후보만 정형 성향 호환도 가중합으로 정렬하고, 동의한 자유 서술 임베딩 유사도는 제한된 보조 점수로 합산합니다.
